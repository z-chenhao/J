...
func TestHostResolvesOnlyExplicitlySelectedObserver(t *testing.T) {
	root := t.TempDir()
	...
	defer session.Close()
	specs, err := session.ObserverSpecs([]string{"dev.usej.observe/trace"})
	if err != nil {
		t.Fatal(err)
	}
	expectedCommand, err := filepath.EvalSymlinks(filepath.Join(root, "observer"))
	if err != nil {
		t.Fatal(err)
	}
	if len(specs) != 1 ||
		specs[0].Name != "dev.usej.observe/trace" ||
		specs[0].Command != expectedCommand ||
		!strings.Contains(strings.Join(specs[0].Env, "\n"), "TRACE_SECRET=secret") {
		t.Fatalf("specs=%+v", specs)
	}
	if _, err := session.ObserverSpecs([]string{"dev.usej.observe/missing"}); err == nil ||
