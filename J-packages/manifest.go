...
// Manifest is the complete experimental J Package 0.1 contract.
...
// Contributions contains only the standard capability types proven in 0.1.
...
func validateManifest(root string, manifest *Manifest) error {
	...
	if len(manifest.Contributes.MCP) == 0 && len(manifest.Contributes.Skills) == 0 {
