...
// and every completed model/tool message. Invalid model output is not committed.
// History returns a defensive snapshot; WithHistory validates and restores such
// a snapshot only during construction. Persistence and session identity remain
// outside the package. Event handlers are synchronous observations and must not
// reenter Run or Reset on the same Agent.
//
